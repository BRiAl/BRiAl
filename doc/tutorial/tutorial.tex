...
\functionname{change\_ordering}(\functionname{Ordering.code}), where \functionname{code} can be either \functionname{lp} (lexicographical ordering), \functionname{dlex} (degree lexicographical ordering), \functionname{dp\_asc} (degree reverse lexicographical ordering with ascending variable ordering), \functionname{block\_dlex} or \functionname{block\_dp\_asc} (for odering composed out of blocks in the corresponding ordering). When using block ordering, after changing to that ordering, blocks have to be defined using the \functionname{append\_ring\_block} function.
...
This can be done quite efficiently using set operations.
\begin{verbatim}
def subst(f,x,c):
   i=x.index()
...
$\PolyBoRi$.
Then we may
reduce the f by this rewriting system
\begin{verbatim}
...
\begin{enumerate}
    \item If our system consists of exactly *one* Boolean polynomial,
    ll\_encode, does essentially only a type conversion (and much overhead).
    This type conversion can be done implicitely (at least using the boost::python interface).
    ...
    In this case, there is no need for calling ll\_encode.
    The second argument is converted implicitely to BooleSet.
    \item A second optimization is to call
    \begin{verbatim}
        ll_red_nf(f,x+c)
    \end{verbatim}
    
    As $\{x+c\}$ is a reduced Gröbner basis (equivalently $\{x+c,\explfieldequations\}\backslash \{x^2+x\}$ is a reduced Gröbner basis).
\end{enumerate}
\subsection{Evaluate a polynomial by plugging in a constant for every variable}
...
We deal here we navigators, which is dangerous, but will show its value in optimized code (e.g. PyRex), which does less overhead.
Note, that they don't increase the reference counts, so we have to make sure,  that f lives  as long as we use an navigator on f.
A second point, why it is desirable to use navigators is, that their thenBranch, elseBranch methods directly calculate the subset0, subset1 results
when calling it with the top variable of the diagram f. In this example, this is the crucial point (in terms of performance),
as we call the polynomial construct on the branches anyway, which does reference counting again.

This is quite fast, but suboptimal, as only the inner functions (additions) use caching.

Furthermore, it works against the usual ZDD recursion and generates complex intermediate results.
\subsubsection{Solution 2: Linear Lead rewriting systems}
The same problem can also be tackled by the ll routines. In this case, when evaluating all variables to constants, the generated intermediate results (during ll\_red\_nf/ll\_red\_nf\_noreds) are only constants.
In general we consider the overhead of generating the encoding d not so big, as this consists only of very few, tiny ZDD operations (and some Python overhead in the quite general ll\_encode).
\begin{verbatim}
...
Since the tails of the polynomials in the rewriting system  only consist of constants, this forms also a
reduced Gröbner basis and you may call
\begin{verbatim}
ll_red_nf(f,d)   
\end{verbatim}


This is assumed to be the fasted way.
