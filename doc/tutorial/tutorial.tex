...
\begin{itemize}
    ...
    \item \verb|clean_and_restart_algorithm|, from time to time restart the algorithm to clean the system of non minimal elements
\end{itemize}
...
In the following, we construct the same polynomial as in \ref{all-monomials-set-operations} directly by elementary \verb|if_then_else| operations. This will save much recursion overhead.
\begin{verbatim}
def all_monomials_of_degree_d(d, variables):
...
(generated during \verb|ll_red_nf_redsb|/\verb|ll_red_nf_noredsb|) are constant.
...
We used \verb|ll_red_nf_redsb|/\verb|ll_red_nf_noredsb| functions on rewriting systems, where the tails of the polynomials was constant and the leading term linear.
...
In particular we can use \verb|ll_red_nf_redsb| to speedup substitution of a variable $x$ by a value $v$ also in the more general case, that the lexicographical leading term of $x+v$ is equal to $x$.
