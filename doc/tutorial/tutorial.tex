...
In addition to polynomials the \PolyBoRi implements a data type for set of monomials, called \functionname{BooleSet}.
This data type is also implemented on top of ZDDs and gives another view on the polynomial  and enables highlevel set operations, which are in most cases faster than operations handling individual terms, as the complexity of the algorithms depends only on the structure of the diagrams.
...
One of the most common operations is to split the set into cofactors of a variable.
...
But also more higher level operations are possible like the calculation of the minimal terms (with respect to division) in a BooleSet:
\begin{verbatim}
In [1]: f=x(2)*x(3)+x(1)*x(3)+x(2)+x(4)
